...
import Library.AlertManagementPage;
import Library.CreateOrderPage;
import Library.EmailReportDCCMO;
...
public class AlertManagement{
	...
	EmailReportDCCMO em = new EmailReportDCCMO();
	...
	int RowNum = 1;
	static String TestDataFileName = "AlertManagement";
	AlertManagementPage alertMgmt = new AlertManagementPage();
	...
	public static Object[][] createOrderProvider() throws FileNotFoundException, IOException {
		excelReader = new Excel_Reader(System.getProperty("user.dir") + "\\Data\\"+System.getProperty("Environment") + "_" +TestDataFileName+".xlsx");
		return asTwoDimensionalArray();
	}
	
	private static Object[][] asTwoDimensionalArray() throws FileNotFoundException, IOException {
		String TestDataPath = 	System.getProperty("user.dir") + "\\Data\\"+System.getProperty("Environment") + "_" +TestDataFileName+".xlsx";
	...
	public void beforeTest1() throws Exception {
		...
		//close the stream
		bwr.close();
		oPSelFW = new ProlificsSeleniumAPI("WCC", "Alert Management", "Chrome", "Windows 7", System.getProperty("Environment"), "Automation",
	...
	public void CreateOrderTestClass() throws FileNotFoundException, IOException, InterruptedException
	{
		XLTestData = excelReader.readExcel("TC_WCC_1");
		if(XLTestData != null)
		{
			InputStream input = new FileInputStream(System.getProperty("user.dir") + "\\resources\\config.properties");
			Properties prop = new Properties();
			prop.load(input);
			oPSelFW.reportStepDetails("Start the New Steps for TestCase", "Alert Management - "+XLTestData.get("TestCaseTitle").toString(), "Pass");
			try {	
				if(loginSuccess)
				{	
			...
			Assert.assertEquals("Verify if exception is displayed", "Exception is displayed in Alert Management");
		}
		}
	}
	@Test(priority = 2, groups = { "smoke", "cov", "reg" })
	public void CreateOrderTestClass2() throws FileNotFoundException, IOException, InterruptedException
	{
		...
		if(XLTestData != null)
		{
			...
			try {	
				if(loginSuccess)
				{	
					alertMgmt.clickOnAlertLink(driver, oPSelFW);
					alertMgmt.searchTheAlertType(driver, "BO Notification Alert", oPSelFW);
					gen.closeTheOrder(driver, oPSelFW);
				}
			}
		catch (Exception e) {
			System.out.println(e);
			oPSelFW.reportStepDetails("Verify if exception is displayed in Alert Search"  , "Exception is displayed in Alert Search", "Fail");
			Assert.assertEquals("Verify if exception is displayed in Alert Search"  , "Exception is displayed in Alert Search");
		}
			
		}
	}
	@Test(priority = 3, groups = { "smoke", "cov", "reg" })
	public void CreateOrderTestClass3() throws FileNotFoundException, IOException, InterruptedException
	{
		XLTestData = excelReader.readExcel("TC_WCC_3");
		if(XLTestData != null)
		{
			InputStream input = new FileInputStream(System.getProperty("user.dir") + "\\resources\\config.properties");
			Properties prop = new Properties();
			...
			try {	
				...
				{	
					alertMgmt.clickOnAlertLink(driver, oPSelFW);
					alertMgmt.searchTheAlertType(driver, "SHAFTER_REVIEW", oPSelFW);
					gen.closeTheOrder(driver, oPSelFW);
				}
			}
		catch (Exception e) {
	...
	public void CreateOrderTestClass4() throws FileNotFoundException, IOException, InterruptedException
	{
		XLTestData = excelReader.readExcel("TC_WCC_4");
		if(XLTestData != null)
		{
			InputStream input = new FileInputStream(System.getProperty("user.dir") + "\\resources\\config.properties");
			Properties prop = new Properties();
			...
			try {	
				...
				{	
					alertMgmt.clickOnAlertLink(driver, oPSelFW);
					alertMgmt.searchTheAlertType(driver, "Pending Payment Authorization", oPSelFW);
					gen.closeTheOrder(driver, oPSelFW);
				}
			}
		catch (Exception e) {
		...
		public void CreateOrderTestClass5() throws FileNotFoundException, IOException, InterruptedException
		{
			XLTestData = excelReader.readExcel("TC_WCC_5");
			if(XLTestData != null)
			{
				InputStream input = new FileInputStream(System.getProperty("user.dir") + "\\resources\\config.properties");
				Properties prop = new Properties();
