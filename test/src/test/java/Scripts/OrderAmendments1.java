...
public class OrderAmendments1{
	...
	public static Object[][] createOrderProvider() throws FileNotFoundException, IOException {
		String Env = System.getProperty("Environment");
		excelReader = new Excel_Reader(System.getProperty("user.dir") + "\\Data\\"+ Env + "_OrderAmendments1.xlsx");
	...
	private static Object[][] asTwoDimensionalArray() throws FileNotFoundException, IOException {
		...
		String Env = System.getProperty("Environment");
		String TestDataPath = System.getProperty("user.dir") + "\\Data\\"+ Env + "_OrderAmendments1.xlsx";
	...
	public void beforeTest1() throws Exception {
		...
		oPSelFW = new ProlificsSeleniumAPI("WCC", "Order Amendments", "Chrome", "Windows 7", "STST", "Automation",
	...
	public void CreateOrderTestClass(HashMap<String, String> XLTestData) throws FileNotFoundException, IOException, InterruptedException
	{
		if(XLTestData != null)
		{
			...
			oPSelFW.reportStepDetails("Start the New Steps for TestCase", "Order Amendments - "+XLTestData.get("TestCaseTitle").toString(), "Pass");
			try {	
				if(loginSuccess)
				{	
						...
						if(errorDisplayed == false)
						{
							...
							if(!OrderCreatedSucc.contains("successfully") || (OrderCreatedSucc.trim().length() == 0))
							{
								...
								if(errorInPayment.trim().length() == 0)
								{
									...
									Thread.sleep(5000);
									String title = XLTestData.get("TestCaseTitle").toString();
									if(title.contains("Order Amendments_Change Bill To_Sales Order"))
									{
									ordercreate1.changeBillingAddress(driver,oPSelFW, XLTestData);
									Thread.sleep(5000);
									ordercreate1.changeBillingAddressValidation(driver, oPSelFW);
									Thread.sleep(5000);
									gen.closeTheOrder(driver, oPSelFW);	
									}
									if(title.contains("Order Amendments_Change Ship To_Item in Existing Gift Order"))
									{
										...
										oPSelFW.reportStepDetails("Get Order Number"  , getOrderNumber + " is generated", "Pass");
										Thread.sleep(5000);
										ordercreate1.changeShipToAddress(driver,oPSelFW, XLTestData);
										Thread.sleep(5000);
										ordercreate1.DoubleCLick(driver, oPSelFW);
										Thread.sleep(5000);
										ordercreate1.changeShipToAddressValidation(driver, oPSelFW);
										gen.closeTheOrder(driver, oPSelFW);
									
									}
									if(title.contains("Order Amendments_Cancellation_Lines of Sub Order"))
									{
										System.out.println("Order Number Generated" +getOrderNumber);
										Thread.sleep(5000);
										ordercreate1.changesuborderaddress(driver, element, oPSelFW, XLTestData );
										//ordercreate.AddPaymentMethod(driver, oPSelFW, element, XLTestData);
										Thread.sleep(5000);
										System.out.println("Order Number Generated" +getOrderNumber);
										Thread.sleep(5000);
										getOrderNumber = driver.findElement(By.xpath("//*[@uid='pnlPrimaryInfo']/div/table/tbody/tr/td/span")).getText();
										oPSelFW.reportStepDetails("Get Order Number"  , getOrderNumber + " is generated", "Pass");
										Thread.sleep(5000);
										ordercreate1.cancelsingleOrder(driver, oPSelFW);
									}
			...
			oPSelFW.reportStepDetails("Verify if exception is displayed", "Exception is displayed in Order Amendments", "Fail");
