...
public class DCCancellation{
	...
	public static Object[][] createOrderProvider() throws FileNotFoundException, IOException {
		String Env = System.getProperty("Environment");
		excelReader = new Excel_Reader(System.getProperty("user.dir") + "\\Data\\"+ Env + "_DCCancellation.xlsx");
	...
	private static Object[][] asTwoDimensionalArray() throws FileNotFoundException, IOException {
		...
		String Env = System.getProperty("Environment");
		String TestDataPath = System.getProperty("user.dir") + "\\Data\\"+ Env + "_DCCancellation.xlsx";
	...
	public void beforeTest1() throws Exception {
		...
		oPSelFW = new ProlificsSeleniumAPI("WCC", "DC Cancellation", "Chrome", "Windows 7", "STST", "Automation",
	...
	public void CreateOrderTestClass(HashMap<String, String> XLTestData) throws FileNotFoundException, IOException, InterruptedException
	{
		if(XLTestData != null)
		{
			...
			oPSelFW.reportStepDetails("Start the New Steps for TestCase", "DC Cancellation - "+XLTestData.get("TestCaseTitle").toString(), "Pass");
			try {	
				if(loginSuccess)
				{	
							...
							if(!OrderCreatedSucc.contains("successfully") || (OrderCreatedSucc.trim().length() == 0))
							{
								...
								if(errorInPayment.trim().length() == 0)
								{
									...
									oPSelFW.reportStepDetails("Get Order Number"  , getOrderNumber + " is generated", "Pass");
										Thread.sleep(5000);
										//gen.waitnclickElement(driver, By.xpath("(//a[contains(text(),'Cancel Order')])[1]"), "Order Cancel", oPSelFW);
									ordercreate1.cancelsingleOrder(driver, oPSelFW);
									Thread.sleep(3000);
									gen.waitnclickElement(driver, By.xpath("(//span[@class='dijitInline dijitTabCloseButton dijitTabCloseIcon'])[2]"), "Close Order", oPSelFW);
									Thread.sleep(30000);
									
									if(XLTestData.get("TestCaseTitle").toString().contains("WS LRC"))
									{
										ordercreate1.ClickBalanceInquiry(driver, element, oPSelFW);
										ordercreate1.BalanceInquirylink(driver, element, oPSelFW);
										Thread.sleep(2000);
										orderPage.DCCancellationBalanceInquery(driver,XLTestData,oPSelFW);
										Thread.sleep(2000);
									}
								}
								else
			...
			oPSelFW.reportStepDetails("Verify if exception is displayed", "Exception is displayed in DC Cancellation", "Fail");
			Assert.assertEquals("Verify if exception is displayed", "Exception is displayed in DC Cancellation");
		}
		}
	}
