...
public class HoldManagement{
	...
	APIMethods callapi = new APIMethods();
	...
	public static Object[][] createOrderProvider() throws FileNotFoundException, IOException {
		String Env = System.getProperty("Environment");
		excelReader = new Excel_Reader(System.getProperty("user.dir") + "\\Data\\"+ Env + "_Holdmanagement.xlsx");
	...
	private static Object[][] asTwoDimensionalArray() throws FileNotFoundException, IOException {
		...
		String Env = System.getProperty("Environment");
		String TestDataPath = System.getProperty("user.dir") + "\\Data\\"+ Env + "_Holdmanagement.xlsx";
		...
		for (int i = 1; i < RowCount; i++) {			
				...
				results[i-1] = new Object[] {XLTestData};
		}
		return results;
	}
	...
	public void beforeTest1() throws Exception {
		...
		oPSelFW = new ProlificsSeleniumAPI("WCC", "Hold Management", "Chrome", "Windows 7", "STST", "Automation",
	...
	public void CreateOrderTestClass(HashMap<String, String> XLTestData) throws FileNotFoundException, IOException, InterruptedException
	{
		if(XLTestData != null)
		{
			...
			oPSelFW.reportStepDetails("Start the New Steps for TestCase", "Hold Management - "+XLTestData.get("TestCaseTitle").toString(), "Pass");
			try {	
				if(loginSuccess)
				{	
						...
						if(errorDisplayed == false)
						{
							...
							if(!OrderCreatedSucc.contains("successfully") || (OrderCreatedSucc.trim().length() == 0))
							{
								...
								if(errorInPayment.trim().length() == 0)
								{
									...
									ordercreate.unLockholds(driver,oPSelFW);
									Thread.sleep(8000);
									
									//ResultSet rs =  callapi.apiStatus(getOrderNumber, XLTestData, oPSelFW);
									String StatusValue = Dbvalidation.getOrderStatus(getOrderNumber, oPSelFW);
								
									//String StatusValue1 = Dbvalidation.getOrderStatus(getOrderNumber, XLTestData, oPSelFW);
									
									Dbvalidation.ScheduleOrderApi(XLTestData, getOrderNumber,StatusValue);
									Thread.sleep(10000);
									
									/*String StatusValueDtc = Dbvalidation.getOrderStatus(OrderNo, envTestData, basetest);
									Dbvalidation.ReleaseOrderApi(XLTestData, StatusValueDtc, OrderNo);
									String StatusValueRelease = Dbvalidation.getOrderStatus(OrderNo, envTestData, basetest);
									Dbvalidation.processOrderDropAck(XLTestData, format_long.format(today), StatusValueRelease, OrderNo, envTestData);
								*/								
								}
								else
			...
			oPSelFW.reportStepDetails("Verify if exception is displayed", "Exception is displayed in Hold Management", "Fail");
			Assert.assertEquals("Verify if exception is displayed", "Exception is displayed in Hold Management");
		}
		}
	}
